...
echo "<h2>Testing Dean Assignment API</h2>";

// First, let's add the title column if it doesn't exist
echo "<h3>1. Checking/Adding Title Column</h3>";

$checkTitleQuery = "SHOW COLUMNS FROM users LIKE 'title'";
$checkResult = $conn->query($checkTitleQuery);

if ($checkResult->num_rows === 0) {
    echo "<p>❌ Title column does not exist. Adding it...</p>";
    
    $addTitleQuery = "ALTER TABLE users ADD COLUMN title VARCHAR(50) DEFAULT NULL AFTER last_name";
    if ($conn->query($addTitleQuery)) {
        echo "<p>✅ Title column added successfully!</p>";
    } else {
        echo "<p>❌ Error adding title column: " . $conn->error . "</p>";
    }
} else {
    echo "<p>✅ Title column already exists</p>";
}

// Test the API endpoint
echo "<h3>2. Testing Dean Assignment API</h3>";
...
if ($deptResult->num_rows > 0) {
    ...
    if ($teacherResult->num_rows > 0) {
        ...
        $apiUrl = "api/assign_department_dean.php";
        ...
        echo "<p>❌ No teachers found in department " . $dept['department_name'] . "</p>";
    }
} else {
...
echo "<h3>3. Current Users Table Structure</h3>";
...
if ($structureResult) {
    ...
    echo "<p>❌ Error describing users table: " . $conn->error . "</p>";
}
