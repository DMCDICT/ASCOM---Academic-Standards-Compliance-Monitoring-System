...
// Modal for switching between user roles (Librarian)
?>
...
// Initialize event listeners only once when DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeSwitchRoleModal);
} else {
    initializeSwitchRoleModal();
}

function initializeSwitchRoleModal() {
    ...
    if (!switchRoleForm || !switchRoleModal || !incorrectPasswordModal) {
        console.error('Switch role modal elements not found');
        return;
    }
    ...
    switchRoleForm.addEventListener('submit', async function(e) {
        ...
        // Get password value and trim (remove leading/trailing whitespace)
        // This handles cases where password is copied from database with extra whitespace
        const rawPassword = document.getElementById('switch_role_password').value;
        const password = rawPassword.trim();
        const targetRole = switchRoleForm.getAttribute('data-target-role');
        const confirmBtn = document.getElementById('confirmSwitchRoleBtn');
        ...
        try {
            ...
            if (data.success) {
                ...
                console.error('Switch role failed:', data.error);
