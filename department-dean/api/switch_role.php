<?php
// switch_role.php
// API endpoint to handle role switching with password verification
...
try {
    ...
    // Use the same approach as teachers/api/switch_role.php
    $user = null;
    $roleInfo = null;
    ...
    if ($password !== $user['password']) {
        ...
        error_log("Switch Role Password Mismatch - User ID: $userId, Target Role: $targetRole");
        error_log("Switch Role Password Mismatch - Input length: " . strlen($password) . ", DB length: " . strlen($user['password']));
        throw new Exception('Invalid password.');
    }
    
    // Set session based on target role (roleInfo already populated above)
    $_SESSION['dean_logged_in'] = false;
